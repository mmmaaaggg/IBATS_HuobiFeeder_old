    ...
    if alter_table:
        with with_db_session(engine=engine_md) as session:
            for table_name, _ in BaseModel.metadata.tables.items():
                ...
                row_data = session.execute(sql_str, params={'table_name': 'md_min1_bc'}).first()
                if row_data is None:
                    continue
                if row_data[1].lower() == 'myisam':
                    continue

                logger.info('修改 %s 表引擎为 MyISAM')
            ...
            session.execute("ALTER TABLE md_min1_tick_bc CHANGE COLUMN `id` `id` INT(11) NULL AUTO_INCREMENT")
